...
export interface McpTokenDelegateLike {
  findUnique(args: { where: { tokenHash: string } }): PromiseLike<unknown>;
  findMany(args: { where: { customerId: string }; orderBy: { createdAt: 'desc' } }): PromiseLike<unknown[]>;
  create(args: {
    data: {
      customerId: string;
      name: string;
  ...
  update(args: { where: { id: string }; data: { lastUsedAt: Date } }): PromiseLike<unknown>;
  updateMany(args: {
    where: { id?: string; customerId: string; revokedAt: null };
    data: { revokedAt: Date };
...
export type McpAuthResult<P> =
  ...
  | { ok: false; error: McpAuthError; code: string; message: string; customerId?: string };

export type McpResolveResult<P> =
  | { ok: true; principal: P }
  | { ok: false; error: 'INVALID' | 'REVOKED' | 'FORBIDDEN'; code: string; message: string };
...
export interface CreateMcpTokensConfig<S extends string, P extends McpPrincipalBase<S>> {
  ...
  resolvePrincipal(ctx: { token: McpTokenRecord & { scopes: S[] }; storeId: string }): Promise<McpResolveResult<P>>;
...
export interface McpTokens<S extends string, P> {
  generate(): { raw: string; prefix: string; tokenHash: string };
  ...
  }): Promise<McpTokenListItem & { raw: string }>;
...
const DELEGATE_METHODS = ['findUnique', 'findMany', 'create', 'update', 'updateMany', 'count'] as const;

const DEFAULT_MESSAGES: Record<McpAuthError, string> = {
...
function shapeError(context: string, field: string): TypeError {
  return new TypeError(
    `[app-core/mcp] ${context}: token row has a missing or wrong-typed "${field}" - ` +
...
function toRecord(value: unknown, context: string): McpTokenRecord {
  if (!value || typeof value !== 'object') throw shapeError(context, 'row');
  const row = value as Record<string, unknown>;
  for (const field of ['id', 'customerId', 'name', 'prefix'] as const) {
    if (typeof row[field] !== 'string' || !row[field]) throw shapeError(context, field);
  }
  if (!Array.isArray(row.scopes)) throw shapeError(context, 'scopes');
  for (const field of ['expiresAt', 'lastUsedAt', 'revokedAt'] as const) {
    if (row[field] !== null && !isValidDate(row[field])) throw shapeError(context, field);
  }
  if (!isValidDate(row.createdAt)) throw shapeError(context, 'createdAt');
  return {
    id: row.id as string,
    customerId: row.customerId as string,
    name: row.name as string,
    prefix: row.prefix as string,
    scopes: (row.scopes as unknown[]).filter((s): s is string => typeof s === 'string'),
...
function toListItem(record: McpTokenRecord): McpTokenListItem {
  return {
    id: record.id,
    name: record.name,
 ...
 */
function requireId(value: unknown, field: 'customerId' | 'tokenId'): string {
...
function readCount(result: unknown, context: string): number {
...
export function createMcpTokens<S extends string, P extends McpPrincipalBase<S>>(
  config: CreateMcpTokensConfig<S, P>
): McpTokens<S, P> {
  ...
  if (
    !Number.isInteger(displayPrefixLength) ||
    displayPrefixLength < tokenPrefix.length ||
    displayPrefixLength > tokenPrefix.length + MAX_EXTRA_DISPLAY_CHARS
  ) {
    throw new TypeError(
      `[app-core/mcp] displayPrefixLength must be between ${tokenPrefix.length} and ${tokenPrefix.length + MAX_EXTRA_DISPLAY_CHARS}`
    );
  }
  ...
  const onError =
    config.onError ?? ((context: string, error: unknown) => console.error(`[app-core/mcp] ${context}:`, error));
  const messageFor = (error: McpAuthError): string => config.messages?.[error] || DEFAULT_MESSAGES[error];
  ...
  function fail(error: McpAuthError, customerId?: string): McpAuthResult<P> {
    return {
      ok: false,
      error,
      code: error,
  ...
  function generate(): { raw: string; prefix: string; tokenHash: string } {
  ...
  function touch(record: McpTokenRecord, at: Date): void {
    const last = record.lastUsedAt;
    if (last && at.getTime() - last.getTime() <= touchIntervalMs) return;
    try {
      tokens
        .update({ where: { id: record.id }, data: { lastUsedAt: at } })
        .then(undefined, (error: unknown) => report('lastUsedAt update failed', error));
    } catch (error) {
      report('lastUsedAt update failed', error);
    }
  }

  async function verify(authorization: string | null | undefined): Promise<McpAuthResult<P>> {
    ...
    if (found === null || found === undefined) return fail('INVALID');
    const record = toRecord(found, 'verify');

    const at = now();
    if (record.revokedAt) return fail('REVOKED', record.customerId);
  ...
  function resolveExpiry(days: unknown): Date | null {
    if (days === null) {
  ...
  async function list(customerId: string): Promise<McpTokenListItem[]> {
    const id = requireId(customerId, 'customerId');
    const rows = await tokens.findMany({ where: { customerId: id }, orderBy: { createdAt: 'desc' } });
    if (!Array.isArray(rows)) throw new TypeError('[app-core/mcp] list: findMany did not return an array');
    return rows.map((row) => toListItem(toRecord(row, 'list')));
  }

  async function countActive(customerId: string): Promise<number> {
    const id = requireId(customerId, 'customerId');
    ...
    if (typeof count !== 'number') throw new TypeError('[app-core/mcp] countActive: count did not return a number');
    return count;
  }

  async function revoke(customerId: string, tokenId: string): Promise<boolean> {
  ...
  async function revokeAll(customerId: string): Promise<number> {
    const owner = requireId(customerId, 'customerId');
    const result = await tokens.updateMany({
